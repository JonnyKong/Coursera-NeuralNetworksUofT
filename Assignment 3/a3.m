...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  hidden_unit_num = size(model.input_to_hid, 1);
  data_num = size(data.inputs, 2);
  delta_1 = zeros(hidden_unit_num, 256);
  delta_2 = zeros(10, hidden_unit_num);
  for i = 1 : data_num
      z2 = model.input_to_hid * data.inputs(:, i);
      a2 = 1 ./ (1 + exp(-1 * z2));
      z3 = model.hid_to_class * a2;
      a3 = exp(z3);
      a3 = a3 / sum(a3);
      sigma3 = a3 - data.targets(:, i);
      sigma2 = (model.hid_to_class)' * sigma3 .* a2 .* (1 - a2);
      %sigma1 = (model.input_to_hid)' * sigma2 .* a1 .* (1 - a1);
      delta_1 = delta_1 + sigma2 * (data.inputs(:, i))';
      delta_2 = delta_2 + sigma3 * (a2)';
  end
  delta_1 = delta_1 / data_num;
  delta_2 = delta_2 / data_num;
  delta_1 = delta_1 + model.input_to_hid * wd_coefficient;
  delta_2 = delta_2 + model.hid_to_class * wd_coefficient;
  ret.input_to_hid = delta_1;
  ret.hid_to_class = delta_2;
end
